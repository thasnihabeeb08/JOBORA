...
# Import UserProfile from current app
...
# ========== JOB SEEKER REGISTRATION FORM - FULLY FIXED ==========
class JobSeekerRegistrationForm(UserCreationForm):
    """Job Seeker Registration Form - COMPLETELY FIXED with salary field"""
    ...
    years_experience = forms.IntegerField(
        ...
        widget=forms.NumberInput(attrs={
            'class': 'form-control',
            'placeholder': '0',
            'step': '1',  # FIXED: Prevents decimal input
            'min': '0',
            'max': '50'
        }),
    ...
    # ========== ADDED SALARY EXPECTATION FIELD ==========
    salary_expectation = forms.IntegerField(
        required=False,
        min_value=0,
        max_value=50000000,
        widget=forms.NumberInput(attrs={
            ...
            'min': '0',
            'max': '50000000',
            'step': '1'  # CRITICAL: Prevents decimal input
        }),
        label="Expected Salary (₹ per annum)",
        help_text="Annual salary expectation in whole rupees. Leave blank if negotiable."
    )
    ...
    def clean_skills(self):
        ...
        if len(skills_list) == 0:
            raise forms.ValidationError('Please enter at least one skill (separated by commas)')
    ...
    def clean_preferred_job_types(self):
        ...
        if len(job_types) == 0:
            raise forms.ValidationError('Please select at least one job type')
        return job_types
    
    # ========== ADDED SALARY VALIDATION ==========
    def clean_salary_expectation(self):
        salary = self.cleaned_data.get('salary_expectation')
        if salary is not None:
            if salary < 0:
                raise forms.ValidationError('Salary cannot be negative')
            if salary > 50000000:
                raise forms.ValidationError('Please enter a realistic salary expectation (max ₹5 Crores)')
        return salary
    
    def save(self, commit=True):
        """Save the user and create profile"""
        # First save the User
        user = super().save(commit=False)
        email = self.cleaned_data['email']
        user.email = email
        ...
        if commit:
            ...
            # Create UserProfile with salary expectation
            profile = UserProfile.objects.create(
                ...
                salary_expectation=self.cleaned_data.get('salary_expectation'),  # ADDED
...
# ========== JOB SEEKER PROFILE EDIT FORM - FIXED ==========
class UserProfileForm(forms.ModelForm):
    """Form for editing job seeker profile - FIXED with salary field"""
    ...
    class Meta:
        ...
        widgets = {
            ...
            'salary_expectation': forms.NumberInput(attrs={
                'class': 'form-control',
                'placeholder': 'e.g., 800000',
                'min': '0',
                'max': '50000000',
                'step': '1'  # CRITICAL: Changed from '10000' to '1'
            }),
...
# ========== APPLICATION FORM - FULLY FIXED ==========
class ApplicationForm(forms.Form):
    """Job Application Form - FIXED with IntegerField"""
    ...
    expected_salary = forms.IntegerField(  # CHANGED FROM DecimalField TO IntegerField
        required=False,
        min_value=0,
        max_value=50000000,
        widget=forms.NumberInput(attrs={
